...
function Particle () {
  this.pos = {
    x: Math.random() * c.width * 0.1 + c.width * 0.45,
    y: Math.random() * c.height * 0.1 + c.height * 0.45
  }
  this.r = 1
  this.speed = 22
  ...
  this.update = function () {
    ...
    if(this.step > 325 || (this.step > 100 && this.step < 190) ) {
      var m = Math.min(c.height, c.width) * 0.3
      var p = Math.PI * 2 * (step + this.step * 0.01) / 180
      this.vx = (Math.cos(p) * m - this.pos.x + c.width/2) * 0.016 + this.vx * 0.97
      this.vy = (Math.sin(p) * m - this.pos.y + c.height/2) * 0.016 + this.vy * 0.97
      //mouse
...
//     }
    ...
    //////////////////////////////////////
    if (this.pos.x > c.width || this.pos.x < 0 || this.pos.y > c.height || this.pos.y < 0) {
      delete this.pos
      delete this.history
      return false;
    }
